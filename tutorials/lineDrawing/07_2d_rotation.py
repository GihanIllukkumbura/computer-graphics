"""
Tutorial 07: 2D Rotation (Matrix Visualization)
================================================

Interactive teaching GUI for 2D rotation about an arbitrary pivot.

Transformation (about pivot px, py):
    x' = cos(theta)*(x-px) - sin(theta)*(y-py) + px
    y' = sin(theta)*(x-px) + cos(theta)*(y-py) + py

Combined homogeneous matrix:
    | cos(theta)  -sin(theta)  px - px*cos(theta) + py*sin(theta) |
    | sin(theta)   cos(theta)  py - px*sin(theta) - py*cos(theta) |
    |    0             0                           1               |

Controls:
    - Left/Right: rotate angle -/+5 deg
    - Shift + Left/Right: fine angle -/+1 deg
    - Mouse click: move pivot
    - R: reset angle and pivot
    - ESC: quit
"""
...
def draw_matrix_block(surface, font, title, matrix, top_left):
    ...
    for row_index, row in enumerate(matrix):
        row_text = "| " + "  ".join(f"{value:>8.3f}" for value in row) + " |"
        surface.blit(font.render(row_text, True, TEXT_COLOR), (x0, y0 + 22 + row_index * 20))


def draw_angle_arc(surface, pivot, angle_degrees):
    """Draw a small arc around pivot to show signed rotation angle."""
...
def main():
    ...
    pygame.display.set_caption("Tutorial 07: 2D Rotation")
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                step = 1.0 if (mods & pygame.KMOD_SHIFT) else 5.0
        ...
        rotation_matrix = [
            [cos_t, -sin_t, pivot[0] - pivot[0] * cos_t + pivot[1] * sin_t],
            [sin_t, cos_t, pivot[1] - pivot[0] * sin_t - pivot[1] * cos_t],
            [0.0, 0.0, 1.0],
        ]

        transformed_polygon = [
            apply_matrix(point, rotation_matrix) for point in BASE_POLYGON
        ]
        ...
        draw_angle_arc(screen, pivot, angle_deg)
        ...
        title = title_font.render("2D Rotation About Pivot", True, TEXT_COLOR)
        hud1 = font.render(
            "Controls: Left/Right = angle | Shift=Fine | Click=Pivot | R=Reset | ESC=Quit",
            True,
            TEXT_COLOR,
        )
        hud2 = font.render(f"Current angle: {angle_deg:.1f} deg", True, TEXT_COLOR)
        hud3 = font.render(
            "Equation: x' = cos(t)(x-px)-sin(t)(y-py)+px, y' = sin(t)(x-px)+cos(t)(y-py)+py",
            True,
            TEXT_COLOR,
        )
        ...
        legend_transformed = font.render("Red: rotated shape", True, TRANSFORM_COLOR)
        ...
        draw_matrix_block(screen, font, "Rotation Matrix (3x3)", rotation_matrix, (720, 20))
