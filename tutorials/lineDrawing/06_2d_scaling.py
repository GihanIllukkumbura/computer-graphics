"""
Tutorial 06: 2D Scaling (Matrix Visualization)
===============================================

Interactive teaching GUI for 2D scaling about an arbitrary pivot.
...
Controls:
    - Left/Right: decrease/increase sx
    - Down/Up: decrease/increase sy
    - Shift + Arrow: fine adjustment
    - Mouse click: move pivot
    - R: reset scale and pivot
    - ESC: quit
"""
...
def clamp_scale(value):
    """Keep scale factors usable while allowing both enlargement and shrinking."""
...
def main():
    ...
    pygame.display.set_caption("Tutorial 06: 2D Scaling")
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    sx = clamp_scale(sx - step)
                elif event.key == pygame.K_RIGHT:
                    sx = clamp_scale(sx + step)
                elif event.key == pygame.K_DOWN:
                    sy = clamp_scale(sy - step)
        ...
        scaling_matrix = [
            [sx, 0.0, pivot[0] * (1.0 - sx)],
            [0.0, sy, pivot[1] * (1.0 - sy)],
            [0.0, 0.0, 1.0],
        ]

        transformed_polygon = [
            apply_matrix(point, scaling_matrix) for point in BASE_POLYGON
        ]
        ...
        title = title_font.render("2D Scaling About Pivot", True, TEXT_COLOR)
        hud1 = font.render(
            "Controls: Left/Right = sx, Down/Up = sy | Shift=Fine | Click=Pivot | R=Reset | ESC=Quit",
            True,
            TEXT_COLOR,
        )
        hud2 = font.render(f"Current scale: sx = {sx:.2f}, sy = {sy:.2f}", True, TEXT_COLOR)
        hud3 = font.render(
            "Equation: x' = px + sx*(x-px),  y' = py + sy*(y-py)",
            True,
            TEXT_COLOR,
        )
        ...
        legend_transformed = font.render("Red: scaled shape", True, TRANSFORM_COLOR)
        ...
        draw_matrix_block(screen, font, "Scaling Matrix (3x3)", scaling_matrix, (760, 20))
